...
public class OnlineCTStores {
	...
	public int discountByItemCount(String[] order, boolean isMember) {
		...
		if(order.length > 10 && isMember == true ) {
			return 10;
		}else if(order.length > 10 && isMember == false) {
			return 5;
		}
 ...
 */
        ...
        String currentItem = "";
		 for(int i=0; i<order.length;i++) {
		    for(int j=i+1; j<order.length; j++) {
			   if(order[i].equals(order[j]) && i != j && !(currentItem.contains(order[i]))) {
				 counter++;
				 currentItem += order[j];
					}
				}
	 ...
	 */
		public String[] buyThreeDiscount(String[] order, boolean isMember) {
			...
			int Spe_Itemcount = 0;
			String freeItem = "";
			List<String> list = new ArrayList(Arrays.asList(order));
	    
	  
			for (int i = 0; i < SPECIAL_ITEMS.length; i++) {
				int count = getNumberOfItemOccurrences(order,SPECIAL_ITEMS[i]);
				if(count >= 3 && isMember == true) {
					Spe_Itemcount += 2;
					freeItem += SPECIAL_ITEMS[i];
				}
	...
	public int discountByItemPrice(String[] order, boolean isMember) {
		//TODO
		for (int i = 0; i < order.length; i++) {
		if(order.length >= 2 && isMember == true && findItemPrice(order[i]) > 50.00 ) {
			return 18;
		}else if(order.length >= 2 && isMember == false && findItemPrice(order[i]) > 75.00 ) {
			return 7;
		}
		}
        return 0;
	...
	public boolean isShippingFree(String[] order, boolean isMember) {
		//TODO
			...
			if( isMember == true && getOrderTotalWithoutDiscount(order) > 75.00 ) {
				return true;
			}else if( isMember == false && getOrderTotalWithoutDiscount(order) > 150.00 ) {
				return true;
			}
		return false;
    }
